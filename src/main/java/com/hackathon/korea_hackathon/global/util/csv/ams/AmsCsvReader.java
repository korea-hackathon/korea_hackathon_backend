...
public class AmsCsvReader {

    /**
     * 학사일정 파일 읽기
     */
    @Bean
    public FlatFileItemReader<ShipLogDTO> amsCsvScheduleReader(){
        /* 파일읽기 */
        FlatFileItemReader<ShipLogDTO> flatFileItemReader = new FlatFileItemReader<>();
        flatFileItemReader.setResource(new ClassPathResource("/file/csv/AMS1.csv")); //읽을 파일 경로 지정
